...
def set_timer_github(workflow_data, target, cron, unset, workflow_file):
    ...
    GitHub Actions schedule events carry no inputs, so we cannot directly schedule
    the per-action workflow generated by FAASR REGISTER (it requires PAYLOAD_URL
    etc.). Instead, we generate a separate workflow that, on schedule, runs the
    same `invoke_workflow.py` script that FAASR INVOKE uses, with the workflow
    JSON filename baked in.
    """
