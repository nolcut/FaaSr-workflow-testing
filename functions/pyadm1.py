...
# Module-level placeholders for data-dependent globals (set by run_simulation)
influent_state = None
initial_state = None
state_input = None
state_zero = None
q_ad = None
S_su = S_aa = S_fa = S_va = S_bu = S_pro = S_ac = S_h2 = S_ch4 = None
S_IC = S_IN = S_I = None
X_xc = X_ch = X_pr = X_li = X_su = X_aa = X_fa = X_c4 = X_pro = X_ac = X_h2 = X_I = None
S_cation = S_anion = None
pH = S_H_ion = S_va_ion = S_bu_ion = S_pro_ion = S_ac_ion = None
S_hco3_ion = S_nh3 = S_nh4_ion = S_co2 = None
S_gas_h2 = S_gas_ch4 = S_gas_co2 = None
S_su_in = S_aa_in = S_fa_in = S_va_in = S_bu_in = S_pro_in = S_ac_in = S_h2_in = None
S_ch4_in = S_IC_in = S_IN_in = S_I_in = None
...
p_gas = p_gas_h2 = p_gas_ch4 = p_gas_co2 = q_gas = q_ch4 = None
K_pH_aa = nn_aa = K_pH_ac = n_ac = K_pH_h2 = n_h2 = None
...
def run_simulation(influent_csv, initial_csv, output_csv):
    """Runs the PyADM1 simulation on the given input files and writes dynamic_out.csv."""
    global influent_state, initial_state, state_zero, state_input, q_ad
    global S_su, S_aa, S_fa, S_va, S_bu, S_pro, S_ac, S_h2, S_ch4
    global S_IC, S_IN, S_I
    global X_xc, X_ch, X_pr, X_li, X_su, X_aa, X_fa, X_c4, X_pro, X_ac, X_h2, X_I
    global S_cation, S_anion
    global pH, S_H_ion, S_va_ion, S_bu_ion, S_pro_ion, S_ac_ion
    global S_hco3_ion, S_nh3, S_nh4_ion, S_co2
    global S_gas_h2, S_gas_ch4, S_gas_co2
    global S_su_in, S_aa_in, S_fa_in, S_va_in, S_bu_in, S_pro_in, S_ac_in, S_h2_in
    global S_ch4_in, S_IC_in, S_IN_in, S_I_in
    global X_xc_in, X_ch_in, X_pr_in, X_li_in, X_su_in, X_aa_in, X_fa_in
    global X_c4_in, X_pro_in, X_ac_in, X_h2_in, X_I_in
    global S_cation_in, S_anion_in
    global p_gas, p_gas_h2, p_gas_ch4, p_gas_co2, q_gas, q_ch4
    ...
    for u in t[1:]:
      ...
      # pandas 2.x compat: DataFrame.append removed; use pd.concat
...
def pyadm1(folder: str, input1: str, input2: str, output1: str) -> None:
    ...
    faasr_log(f"pyadm1: downloaded {input1} and {input2} from {folder}")

    faasr_log("pyadm1: starting PyADM1 simulation")
    run_simulation(local_inf, local_ini, local_out)
    faasr_log("pyadm1: simulation complete")

    faasr_put_file(local_file=local_out, remote_folder=folder, remote_file=output1)
    faasr_log(f"pyadm1: uploaded {output1} to {folder}")
