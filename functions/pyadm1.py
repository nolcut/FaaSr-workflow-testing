...
# The original script's top-level runtime logic, enclosed VERBATIM in a callable
# function so it can be invoked by the FaaSr entry point. Every statement below is
# copied unchanged from functions/original_pyadm1.py; the only additions are the
# `global` declarations required so that these top-level names remain module globals
# (exactly as they were when executed at module scope in the original script), which
# the model functions above read/write via their own `global` statements. No model
# logic, constants, or output formatting are altered.
def run_pyadm1_model():
  ...
  global q_ad, state_zero, state_input, p_gas, q_gas, q_ch4, p_gas_h2, P_gas
...
# ---------------------------------------------------------------------------
# FaaSr entry point. Handles S3 I/O and delegates ALL computation to the
# preserved PyADM1 model above (run_pyadm1_model, which reads the two input
# CSVs and writes dynamic_out.csv exactly as the original script did).
# ---------------------------------------------------------------------------
def pyadm1(folder: str, input1: str, input2: str, output1: str) -> None:
    faasr_log(
        f"pyadm1: starting PyADM1 simulation for '{input1}' and '{input2}' "
        f"in folder '{folder}'."
    )
    ...
    # The preserved model reads these EXACT hardcoded filenames from the cwd.
    local_influent = os.path.join(workdir, "digester_influent.csv")
    local_initial = os.path.join(workdir, "digester_initial.csv")

    # Download the two validated input CSVs from S3.
    ...
    for path, name in ((local_influent, input1), (local_initial, input2)):
        if not os.path.exists(path) or os.path.getsize(path) == 0:
            msg = (
                f"pyadm1: required input file '{name}' could not be retrieved "
                f"from folder '{folder}' (missing or empty)."
            )
            faasr_log(msg)
            raise FileNotFoundError(msg)

    faasr_log("pyadm1: inputs downloaded; running the PyADM1 dynamic simulation.")

    # Run the preserved model in the working directory so its hardcoded
    # relative paths resolve to the downloaded inputs / produced output.
    try:
        os.chdir(workdir)
        run_pyadm1_model()
    finally:
    ...
    out_local = os.path.join(workdir, "dynamic_out.csv")
    if not os.path.exists(out_local) or os.path.getsize(out_local) == 0:
        msg = "pyadm1: PyADM1 simulation did not produce a valid 'dynamic_out.csv'."
        faasr_log(msg)
        raise RuntimeError(msg)

    faasr_put_file(local_file=out_local, remote_folder=folder, remote_file=output1)
    faasr_log(
        f"pyadm1: simulation complete. Dynamic ADM1 state time-series written "
        f"to '{output1}' in folder '{folder}'."
    )
