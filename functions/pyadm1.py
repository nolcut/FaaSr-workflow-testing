# ---------------------------------------------------------------------------
# FaaSr wrapper for the user-provided PyADM1 model.
#
# The user's original model script (functions/original_pyadm1.py) is preserved
# VERBATIM below: its complete top-level logic is enclosed UNCHANGED inside
# run_model() (only a uniform 4-space indent was added so it is callable). No
# constant, algorithm, or output-format statement was modified.
#
# The original ran at module scope, where its top-level statements and its
# nested helper functions (setInfluent/ADM1_ODE/simulate/DAESolve) shared one
# namespace via `global`. To reproduce that EXACT semantics inside run_model(),
# the enclosing function re-declares `global` for precisely the names the
# helpers declare `global` (extracted verbatim from the model) so the enclosed
# statements bind to the same shared namespace they originally did. This adds a
# declaration only; it changes no model statement.
#
# The FaaSr entry function pyadm1() produces results ONLY by calling run_model().
# ---------------------------------------------------------------------------
...
def pyadm1(folder: str, input1: str, input2: str, output1: str) -> None:
    faasr_log("pyadm1: starting PyADM1 simulation for folder '%s'" % folder)

    # The preserved model reads these exact filenames from the working directory
    # (pd.read_csv("digester_influent.csv") / pd.read_csv("digester_initial.csv"))
    # and writes its results to dynamic_out.csv.
    faasr_log("Downloading influent file '%s' -> digester_influent.csv" % input1)
    faasr_get_file(local_file="digester_influent.csv", remote_folder=folder, remote_file=input1)

    faasr_log("Downloading initial-state file '%s' -> digester_initial.csv" % input2)
    faasr_get_file(local_file="digester_initial.csv", remote_folder=folder, remote_file=input2)

    # Produce results ONLY by invoking the preserved user model.
    faasr_log("Running the preserved PyADM1 model (run_model)...")
    run_model()
    faasr_log("PyADM1 model run complete; produced dynamic_out.csv")

    # Upload the model's output under the declared output filename.
    faasr_put_file(local_file="dynamic_out.csv", remote_folder=folder, remote_file=output1)
    faasr_log("Uploaded PyADM1 simulation results to '%s'" % output1)
