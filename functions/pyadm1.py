"""FaaSr wrapper for the user-provided PyADM1 dynamic simulation model.

The user's original model (functions/original_pyadm1.py) is reproduced VERBATIM
below inside run_model(): every statement's logic is unchanged. The only
structural change permitted by the task is enclosing the plain script in a
callable function. The leading `global` declaration lists exactly the names the
model's own functions (setInfluent / ADM1_ODE / DAESolve) share via `global`, so
the script's module-global state coupling behaves identically to the original.

The FaaSr entry function `pyadm1` downloads the inputs, calls the preserved
model, and uploads its result. It contains NO model logic of its own.
"""
import os as _os

# The model imports matplotlib.pyplot; force a headless backend for the
# container. This is wrapper setup, not a change to the model's logic.
_os.environ.setdefault("MPLBACKEND", "Agg")


def run_model():
    global S_su_in, S_aa_in, S_fa_in, S_va_in, S_bu_in, S_pro_in, S_ac_in, S_h2_in, S_ch4_in, S_IC_in, S_IN_in, S_I_in, X_xc_in, X_ch_in, X_pr_in, X_li_in, X_su_in, X_aa_in, X_fa_in, X_c4_in, X_pro_in, X_ac_in, X_h2_in, X_I_in, S_cation_in, S_anion_in, S_nh4_ion, S_co2, p_gas, q_gas, q_ch4, S_va_ion, S_bu_ion, S_pro_ion, S_ac_ion, S_hco3_ion, S_nh3, S_H_ion, pH, p_gas_h2, S_h2, P_gas
    import numpy as np
...
def _ensure_pandas_append():
    ...
    preserved model uses. Restore an equivalent (backed by pd.concat) WITHOUT
    touching the model source. On pandas < 2.0 the native method exists and this
    is a no-op."""
    import pandas as pd
...
def pyadm1(folder: str, input1: str, input2: str, output1: str) -> None:
    """FaaSr entry point: run the PyADM1 dynamic simulation.

    Downloads the cleaned influent (input1) and initial-conditions (input2) CSVs
    from the S3 folder, invokes the preserved model (run_model), and uploads the
    resulting time-series simulation to output1. Results are produced ONLY by
    calling run_model() -- no simulation logic lives in this wrapper.
    """
    import os
    import shutil
    import tempfile

    # The preserved model reads these exact hardcoded filenames from the current
    # working directory and writes "dynamic_out.csv" there.
    MODEL_INFLUENT = "digester_influent.csv"
    MODEL_INITIAL = "digester_initial.csv"
    MODEL_OUTPUT = "dynamic_out.csv"
    ...
    prev_cwd = os.getcwd()
    try:
        faasr_get_file(local_file=os.path.join(workdir, MODEL_INFLUENT),
                       remote_folder=folder, remote_file=input1)
        faasr_get_file(local_file=os.path.join(workdir, MODEL_INITIAL),
                       remote_folder=folder, remote_file=input2)

        _ensure_pandas_append()

        os.chdir(workdir)
        faasr_log("pyadm1: running PyADM1 dynamic simulation (run_model)")
        run_model()

        produced = os.path.join(workdir, MODEL_OUTPUT)
        if not os.path.exists(produced):
            msg = (f"pyadm1: model did not produce expected output "
                   f"'{MODEL_OUTPUT}'")
            faasr_log(msg)
            raise RuntimeError(msg)

        os.chdir(prev_cwd)
        faasr_log(f"pyadm1: uploading simulation output to {output1}")
        faasr_put_file(local_file=produced, remote_folder=folder,
                       remote_file=output1)
