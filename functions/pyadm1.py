...
# ============================================================================
# User-provided model, copied VERBATIM from functions/original_pyadm1.py.
# The original is a plain script whose top-level statements rely on
# module-global scope (its inner functions use `global` declarations that bind
# to the script's module namespace). To keep it callable WITHOUT altering any
# statement's logic, the verbatim source is held below and executed in a fresh
# global namespace by run_pyadm1_model() -- this reproduces running the script
# exactly (identical scoping and results), which a function-nested copy would
# not. Not one character of the model's logic is changed.
# ============================================================================
_PYADM1_MODEL_SOURCE = r"""
import numpy as np
...
def run_pyadm1_model():
    """Execute the user's original PyADM1 script verbatim in a fresh global
    namespace. It reads 'digester_influent.csv' and 'digester_initial.csv' from
    the current working directory and writes 'dynamic_out.csv' there, exactly as
    the standalone script does."""
    # Headless container has no display; ensure matplotlib uses a non-GUI
    # backend before the model imports pyplot (does not change model logic).
    os.environ.setdefault("MPLBACKEND", "Agg")
    model_globals = {"__name__": "__main__", "__builtins__": __builtins__}
    exec(compile(_PYADM1_MODEL_SOURCE, "original_pyadm1.py", "exec"), model_globals)


def pyadm1(folder: str, input1: str, input2: str, output1: str) -> None:
    """FaaSr entry point. Produces results ONLY by calling the preserved model
    run_pyadm1_model(); it contains no simulation logic of its own.

    The model reads 'digester_influent.csv' and 'digester_initial.csv' by fixed
    basenames from the working directory and writes 'dynamic_out.csv' there, so
    we stage the S3 inputs under exactly those names, invoke the model, then
    upload the produced CSV as output1.
    """
    faasr_log(f"pyadm1: fetching inputs '{input1}', '{input2}' from folder '{folder}'")
    ...
    faasr_log("pyadm1: running PyADM1 dynamic simulation")
    run_pyadm1_model()

    faasr_log(f"pyadm1: uploading simulation results to '{output1}'")
