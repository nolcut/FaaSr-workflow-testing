...
def run_adm1_simulation(influent_csv, initial_csv, output_csv):
    # reading influent and initial condition data from csv files
    influent_state = pd.read_csv(influent_csv)
    initial_state = pd.read_csv(initial_csv)

    # Shared mutable state container for variables exchanged between nested functions
    # (replaces module-level globals from the original standalone script)
    _sv = {}
    ...
    def setInfluent(i):
        ...
        _sv['S_su_in'] = influent_state['S_su'][i] #kg COD.m^-3
        _sv['S_aa_in'] = influent_state['S_aa'][i] #kg COD.m^-3
        _sv['S_fa_in'] = influent_state['S_fa'][i] #kg COD.m^-3
        _sv['S_va_in'] = influent_state['S_va'][i] #kg COD.m^-3
        _sv['S_bu_in'] = influent_state['S_bu'][i] #kg COD.m^-3
        _sv['S_pro_in'] = influent_state['S_pro'][i] #kg COD.m^-3
        _sv['S_ac_in'] = influent_state['S_ac'][i] #kg COD.m^-3
        _sv['S_h2_in'] = influent_state['S_h2'][i] #kg COD.m^-3
        _sv['S_ch4_in'] = influent_state['S_ch4'][i]  #kg COD.m^-3
        _sv['S_IC_in'] = influent_state['S_IC'][i] #kmole C.m^-3
        _sv['S_IN_in'] = influent_state['S_IN'][i] #kmole N.m^-3
        _sv['S_I_in'] = influent_state['S_I'][i] #kg COD.m^-3

        _sv['X_xc_in'] = influent_state['X_xc'][i] #kg COD.m^-3
        _sv['X_ch_in'] = influent_state['X_ch'][i] #kg COD.m^-3
        _sv['X_pr_in'] = influent_state['X_pr'][i] #kg COD.m^-3
        _sv['X_li_in'] = influent_state['X_li'][i] #kg COD.m^-3
        _sv['X_su_in'] = influent_state['X_su'][i] #kg COD.m^-3
        _sv['X_aa_in'] = influent_state['X_aa'][i] #kg COD.m^-3
        _sv['X_fa_in'] = influent_state['X_fa'][i] #kg COD.m^-3
        _sv['X_c4_in'] = influent_state['X_c4'][i] #kg COD.m^-3
        _sv['X_pro_in'] = influent_state['X_pro'][i] #kg COD.m^-3
        _sv['X_ac_in'] = influent_state['X_ac'][i] #kg COD.m^-3
        _sv['X_h2_in'] = influent_state['X_h2'][i] #kg COD.m^-3
        _sv['X_I_in'] = influent_state['X_I'][i] #kg COD.m^-3

        _sv['S_cation_in'] = influent_state['S_cation'][i] #kmole.m^-3
        _sv['S_anion_in'] = influent_state['S_anion'][i] #kmole.m^-3
    ...
    def DAESolve():
      nonlocal S_va_ion, S_bu_ion, S_pro_ion, S_ac_ion, S_hco3_ion, S_nh3, S_H_ion, pH, S_h2, S_nh4_ion, S_co2
      ...
      while ((S_h2delta > tol or S_h2delta < -tol) and (j <= maxIter)):
        ...
        p_gas_h2 = S_gas_h2 * R * T_ad / 16
        Rho_T_8 = k_L_a * (S_h2 - 16 * K_H_h2 * p_gas_h2)
        S_h2delta = q_ad / V_liq * (_sv['S_h2_in'] - S_h2) + (1 - Y_su) * f_h2_su * Rho_5 + (1 - Y_aa) * f_h2_aa * Rho_6 + (1 - Y_fa) * 0.3 * Rho_7 + (1 - Y_c4) * 0.15 * Rho_8 + (1 - Y_c4) * 0.2 * Rho_9 + (1 - Y_pro) * 0.43 * Rho_10 - Rho_12 - Rho_T_8
    ...
    for u in t[1:]:
      ...
      state_input = [_sv['S_su_in'],_sv['S_aa_in'],_sv['S_fa_in'],_sv['S_va_in'],_sv['S_bu_in'],_sv['S_pro_in'],_sv['S_ac_in'],_sv['S_h2_in'],_sv['S_ch4_in'],_sv['S_IC_in'],_sv['S_IN_in'],_sv['S_I_in'],_sv['X_xc_in'],_sv['X_ch_in'],_sv['X_pr_in'],_sv['X_li_in'],_sv['X_su_in'],_sv['X_aa_in'],_sv['X_fa_in'],_sv['X_c4_in'],_sv['X_pro_in'],_sv['X_ac_in'],_sv['X_h2_in'],_sv['X_I_in'],_sv['S_cation_in'],_sv['S_anion_in']]
      ...
      flowtemp = pd.DataFrame([{'q_gas' : q_gas, 'q_ch4' : q_ch4}])
      gasflow = pd.concat([gasflow, flowtemp], ignore_index=True)
    ...
    simulate_results.to_csv(output_csv, index = False)


def pyadm1(folder: str, input1: str, input2: str, output1: str) -> None:
    faasr_log("pyadm1: downloading input files")
    ...
    faasr_log("pyadm1: running ADM1 simulation")
    run_adm1_simulation("digester_influent.csv", "digester_initial.csv", "dynamic_out.csv")

    faasr_log("pyadm1: uploading output")
    faasr_put_file(local_file="dynamic_out.csv", remote_folder=folder, remote_file=output1)
    faasr_log("pyadm1: complete")
