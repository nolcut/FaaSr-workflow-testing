...
def map(folder: str, input1: str, output1: str) -> None:
    ...
    if not os.path.exists("chunk_{rank}.txt"):
        faasr_log("[REQUIRE] CONTRACT VIOLATION: Input text chunk for this rank must exist on S3 before mapping")
        raise SystemExit(1)
    if not os.path.exists("chunk_{rank}.txt") or os.path.getsize("chunk_{rank}.txt") == 0:
        faasr_log("[REQUIRE] CONTRACT VIOLATION: Input text chunk for this rank must not be empty — nothing to count")
        raise SystemExit(1)
    # --- end requires ---

    if not os.path.exists(local_chunk) or os.path.getsize(local_chunk) == 0:
        msg = f"map[{rank}]: chunk file {chunk_remote} is missing or empty"
        faasr_log(msg)
        raise ValueError(msg)

    # ------------------------------------------------------------------ #
    # 3. Read text and tokenise into normalised words                      #
    # Normalisation: lowercase + strip leading/trailing punctuation.       #
    # ------------------------------------------------------------------ #
    ...
    if not os.path.exists("partial_counts_{rank}.json"):
        faasr_log("[PROMISE] CONTRACT VIOLATION: Partial word-count file must exist on S3 after mapping completes")
        raise SystemExit(1)
    if not os.path.exists("partial_counts_{rank}.json") or os.path.getsize("partial_counts_{rank}.json") == 0:
        faasr_log("[PROMISE] CONTRACT VIOLATION: Partial word-count JSON must not be empty after mapping completes")
        raise SystemExit(1)
    try:
        import json as _json; _json.loads(open("partial_counts_{rank}.json").read())
    except Exception as _e:
        faasr_log("[PROMISE] CONTRACT VIOLATION: Partial word-count output must be valid JSON: " + str(_e))
        raise SystemExit(1)
    # --- end promises ---
    faasr_put_file(
