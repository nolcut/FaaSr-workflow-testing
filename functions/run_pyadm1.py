...
# =============================================================================
# PyADM1 (Rosen et al. 2006 BSM2 implementation of ADM1).
# Adapted from the user-provided PyADM1.py so that it runs as a FaaSr function:
#   * CSV reading / initial-state setup / the time loop are moved inside
#     run_pyadm1() instead of executing at import time.
#   * Deprecated DataFrame.append() calls are replaced with pd.concat().
# The biochemistry (parameters, ADM1_ODE, DAESolve) is unchanged.
# =============================================================================
...
# --- Physical parameters used in BSM2 ---------------------------------------
V_liq = 3400
V_gas = 300
V_ad = V_liq + V_gas

# --- pH inhibition constants (globals to avoid repeating them in the DAE) ----
...
def setInfluent(i):
    """Set the influent (feed) values for time step i from influent_state."""
    global S_su_in, S_aa_in, S_fa_in, S_va_in, S_bu_in, S_pro_in, S_ac_in
    global S_h2_in, S_ch4_in, S_IC_in, S_IN_in, S_I_in, X_xc_in, X_ch_in
    global X_pr_in, X_li_in, X_su_in, X_aa_in, X_fa_in, X_c4_in, X_pro_in
    global X_ac_in, X_h2_in, X_I_in, S_cation_in, S_anion_in
...
def ADM1_ODE(t, state_zero):
    """ADM1 system of derivatives (Rosen et al. 2006 BSM2 report)."""
    ...
    # diff_S_h2 is solved via the DAE routine (kept at 0 here)
    ...
    # ion states (solved by DAE, held at 0 in the ODE)
    diff_S_va_ion = 0
    diff_S_bu_ion = 0
    diff_S_pro_ion = 0
    diff_S_ac_ion = 0
...
def simulate(t_step, solvermethod):
    """Integrate the ADM1 ODE system over one time step."""
    r = scipy.integrate.solve_ivp(ADM1_ODE, t_step, state_zero, method=solvermethod)
    return r.y


def DAESolve():
    """Solve the algebraic (acid-base and hydrogen) equations (Rosen et al. 2006)."""
    global S_va_ion, S_bu_ion, S_pro_ion, S_ac_ion, S_hco3_ion, S_nh3, S_H_ion
    global pH, p_gas_h2, S_h2, S_nh4_ion, S_co2, P_gas, q_gas
...
def run_pyadm1(folder, influent_file, initial_file, output_file):
    """
    FaaSr entry point: run the PyADM1 dynamic anaerobic-digestion simulation.

    Downloads the (already validated) influent time series and initial reactor
    state from S3, integrates the ADM1 system across every influent time step,
    writes the dynamic state trajectory to output_file, and uploads it to S3.
    """
    global influent_state, initial_state, state_input, state_zero, q_ad, t
    ...
    global S_cation, S_anion, S_H_ion, S_va_ion, S_bu_ion, S_pro_ion, S_ac_ion
    global S_hco3_ion, S_co2, S_nh3, S_nh4_ion, S_gas_h2, S_gas_ch4, S_gas_co2
    global pH, p_gas_h2, p_gas_ch4, p_gas_co2, p_gas, q_gas, q_ch4
    ...
    faasr_log(f"run_pyadm1: loaded {len(influent_state)} influent time steps")

    # --- initial reactor state (t0) ------------------------------------------
    ...
    state_zero = [S_su, S_aa, S_fa, S_va, S_bu, S_pro, S_ac, S_h2, S_ch4, S_IC,
                  S_IN, S_I, X_xc, X_ch, X_pr, X_li, X_su, X_aa, X_fa, X_c4,
                  X_pro, X_ac, X_h2, X_I, S_cation, S_anion, S_H_ion, S_va_ion,
    ...
    # --- dynamic simulation over each influent time step ---------------------
    for u in t[1:]:
        n += 1
        setInfluent(n)
        ...
        (sim_S_su, sim_S_aa, sim_S_fa, sim_S_va, sim_S_bu, sim_S_pro, sim_S_ac,
         sim_S_h2, sim_S_ch4, sim_S_IC, sim_S_IN, sim_S_I, sim_X_xc, sim_X_ch,
         sim_X_pr, sim_X_li, sim_X_su, sim_X_aa, sim_X_fa, sim_X_c4, sim_X_pro,
         sim_X_ac, sim_X_h2, sim_X_I, sim_S_cation, sim_S_anion, sim_S_H_ion,
         sim_S_va_ion, sim_S_bu_ion, sim_S_pro_ion, sim_S_ac_ion,
         sim_S_hco3_ion, sim_S_co2, sim_S_nh3, sim_S_nh4_ion, sim_S_gas_h2,
        ...
        (S_su, S_aa, S_fa, S_va, S_bu, S_pro, S_ac, S_h2, S_ch4, S_IC, S_IN,
         ...
         S_gas_co2) = (
            sim_S_su[-1], sim_S_aa[-1], sim_S_fa[-1], sim_S_va[-1], sim_S_bu[-1],
            sim_S_pro[-1], sim_S_ac[-1], sim_S_h2[-1], sim_S_ch4[-1],
            sim_S_IC[-1], sim_S_IN[-1], sim_S_I[-1], sim_X_xc[-1], sim_X_ch[-1],
            sim_X_pr[-1], sim_X_li[-1], sim_X_su[-1], sim_X_aa[-1], sim_X_fa[-1],
            sim_X_c4[-1], sim_X_pro[-1], sim_X_ac[-1], sim_X_h2[-1], sim_X_I[-1],
            sim_S_cation[-1], sim_S_anion[-1], sim_S_H_ion[-1], sim_S_va_ion[-1],
            sim_S_bu_ion[-1], sim_S_pro_ion[-1], sim_S_ac_ion[-1],
            sim_S_hco3_ion[-1], sim_S_co2[-1], sim_S_nh3[-1], sim_S_nh4_ion[-1],
        ...
        state_zero = [S_su, S_aa, S_fa, S_va, S_bu, S_pro, S_ac, S_h2, S_ch4,
                      ...
                      S_hco3_ion, S_co2, S_nh3, S_nh4_ion, S_gas_h2, S_gas_ch4,
                      S_gas_co2]
    ...
    # The 'pH' column above actually stores S_H_ion; convert to true pH.
    simulate_results['pH'] = -1 * np.log10(simulate_results['pH'])

    # Attach the time axis and gas flows for convenient downstream plotting.
    gasflow = pd.DataFrame(gasflow_rows)
    simulate_results.insert(0, 'time', t.values)
    simulate_results['q_gas'] = gasflow['q_gas'].values
    simulate_results['q_ch4'] = gasflow['q_ch4'].values

    simulate_results.to_csv(output_file, index=False)
    faasr_put_file(local_file=output_file, remote_folder=folder,
                   remote_file=output_file)
    faasr_log(
        f"run_pyadm1: simulation complete over {len(simulate_results)} steps; "
        f"total CH4 = {total_ch4:.3f}; wrote {output_file} to {folder}"
    )
