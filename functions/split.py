...
def _faasr_promises(folder):
    if "manifest.json" not in [_k.rsplit("/", 1)[-1] for _k in faasr_get_folder_list(prefix=folder)]:
        faasr_log("[PROMISE] CONTRACT VIOLATION: Manifest file 'manifest.json' was not found in S3 after split completed")
        raise SystemExit(1)
...
def split(folder: str, input1: str, output1: str, output2: str) -> None:
    ...
    faasr_log(f"Downloading input file '{input1}' from folder '{folder}'")
